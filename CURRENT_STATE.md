...
* AI assistant emits structured `BookCitation[]` via a `{ type: 'books', books }` SSE event before `[DONE]`. Citations are extracted from `searchForAgent()` results (canonical IDs from DB, never authored by the LLM) and attached to `ChatMessage.citations`. The frontend renders them as trusted Next.js `<Link>` cards (`BookCitationCards`) below the assistant text, bypassing the markdown rendering path for book navigation. The catalog `[id]` not-found page now shows a "Search Catalog" button alongside "Go Back". References: [agent.service.ts](/C:/Projects/library-system_edit/apps/api/src/ai/agent.service.ts), [ai.controller.ts](/C:/Projects/library-system_edit/apps/api/src/ai/ai.controller.ts), [BookCitationCards.tsx](/C:/Projects/library-system_edit/apps/web/components/BookCitationCards.tsx), [ai-assistant/page.tsx](/C:/Projects/library-system_edit/apps/web/app/dashboard/ai-assistant/page.tsx)
